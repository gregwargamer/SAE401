...
const LogementPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      const [geoResult, stats] = await Promise.all([
        fetchGeoData(),
        apiClient.get("/all"),
      ]);
  ...
  const latestData = useMemo(() => {
    ...
    const anneeMax = Math.max(
      ...rawData.map((s) => Number(s.annee)).filter((n) => !isNaN(n)),
    );
    return rawData
      .filter((s) => Number(s.annee) === anneeMax)
      .map((d) => ({
        ...d,
        nom: d.nom_departement || d.nom,
        code: d.code_departement || d.code,
      }));
  }, [rawData]);
  ...
  const mapData = useMemo(() => {
    return geoData
      .map((g) => {
        let geometry;
        try {
          geometry = typeof g.geom === "string" ? JSON.parse(g.geom) : g.geom;
        } catch (e) {
          return null;
        }
  ...
  const scatter1Data = useMemo(() => {
    let filtered = latestData.filter(
      (d) =>
        !isNaN(Number(d.densite)) && !isNaN(Number(d.taux_logements_sociaux)),
    );
  ...
  const scatter1Options = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx) =>
            `${ctx.raw.nom}: ${ctx.raw.x} hab/km² | ${ctx.raw.y}% log. soc.`,
        },
      },
    },
    scales: {
      x: {
        ...
        position: "bottom",
        ticks: {
          callback: (val) => ([10, 100, 1000, 10000].includes(val) ? val : ""),
        },
      },
      y: {
        title: {
          display: true,
          text: "Taux log. sociaux (%)",
          font: { size: 11, weight: "600" },
        },
      },
    },
  ...
  const barData = useMemo(() => {
    ...
    return {
      labels: sliced.map(
        (d) => d.nom.substring(0, 12) + (d.nom.length > 12 ? "." : ""),
      ),
      datasets: [
        {
          label: "Individuel (%)",
  ...
  const scatter2Data = useMemo(() => {
    const points = latestData
      .filter(
        (d) =>
          !isNaN(Number(d.moyenne_construction_neuve || d.construction)) &&
          !isNaN(Number(d.taux_logements_vacants)),
      )
      .map((d) => ({
  ...
  const scatter2Options = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx) =>
            `${ctx.raw.nom}: ${ctx.raw.x}% vacants | ${ctx.raw.y} constr.`,
        },
      },
    },
    scales: {
      ...
      <div className="flex-1 ml-0 xl:ml-[22%] flex flex-col gap-8 p-6 xl:p-8 xl:pt-0">
        ...
        <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col">
            <h3 className="text-md font-bold text-slate-800 mb-1 leading-snug">
              Palmarès : Taux de logements sociaux
            </h3>
            <p className="text-xs text-slate-500 mb-4 line-clamp-1">
              Les 5 départements les plus et les moins bien dotés.
            </p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5 flex flex-col">
            <h3 className="text-md font-bold text-slate-800 mb-1 leading-snug">
              Logements individuels vs collectifs
            </h3>
            <p className="text-xs text-slate-500 mb-4 line-clamp-1">
              Structure du parc par département, opposant l'habitat
        ...
        <div className="w-full flex flex-col lg:flex-row gap-6">
          <div className="w-full lg:w-3/4 bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 className="text-md font-bold text-slate-800 mb-1">
              Densité vs Taux de logements sociaux
            </h3>
            <p className="text-xs text-slate-500 mb-4">
              Montre que les zones denses concentrent le parc social, lecture
          ...
          <div className="w-full lg:w-1/4 flex flex-col justify-center px-2">
            <h4 className="text-2xl font-extrabold text-slate-800 mb-3">
              Concentration urbaine
            </h4>
            <p className="text-sm text-slate-600 mb-3 leading-relaxed">
              Le parc social est structurellement dépendant de la densité de
              population.
            </p>
            <p className="text-sm text-slate-600 leading-relaxed">
              Plus un territoire est dense, plus le taux de logements sociaux
              grimpe pour répondre aux besoins d'une population concentrée aux
              revenus parfois plus précaires.
            </p>
          </div>
        </div>
        ...
        <div className="w-full flex flex-col lg:flex-row gap-6">
          <div className="w-full lg:w-1/4 flex flex-col justify-center px-2 order-2 lg:order-1">
            <h4 className="text-2xl font-extrabold text-slate-800 mb-3">
              La contradiction territoriale
            </h4>
            <p className="text-sm text-slate-600 mb-3 leading-relaxed">
              Ce graphique révèle les fractures locales les plus fortes.
            </p>
            <p className="text-sm text-slate-600 leading-relaxed">
              Les zones où l'on construit abondamment absorbent mal le
              trop-plein de vacance des territoires ruraux périphériques,
              témoignant d'une inadéquation entre offre et demande.
            </p>
          </div>
          <div className="w-full lg:w-3/4 bg-white rounded-xl shadow-sm border border-slate-200 p-5 order-1 lg:order-2">
            <h3 className="text-md font-bold text-slate-800 mb-1">
              Construction neuve × Taux de logements vacants
            </h3>
            <p className="text-xs text-slate-500 mb-4">
              Visualisation de la déconnexion entre production et occupation
            </p>
            <div className="h-[280px] w-full">
