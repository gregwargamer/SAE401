...
const TestPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const anneeMax = Math.max(
        ...statsList.map((s) => Number(s.annee)).filter((n) => !isNaN(n)),
      );
      ...
      setData(
        geoResult.map((g) => {
          const sesChiffres =
            statsAjour.find((s) => s.code_departement === g.code) || {};
      ...
      .map((d) => {
        ...
        return {
          ...
          properties: {
            nom: d.nom,
            code: d.code,
            taux_logements_sociaux: d.taux_logements_sociaux,
            taux_chomage: d.taux_chomage,
  ...
  const scatterOptions = {
    ...
    scales: {
      x: {
        ...
        grid: { color: "#e5e7eb" },
        type: "logarithmic",
        position: "bottom", // L'échelle logarithmique règle le problème d'écrasement des points
        ticks: {
  ...
  const barChartData = useMemo(() => {
    ...
    const dataMetropole = data.filter(
      (d) =>
    ...
    const sorted = [...dataMetropole].sort(
      (a, b) =>
        Number(b.taux_logements_sociaux) - Number(a.taux_logements_sociaux),
    );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-6 items-start">
        ...
        <div className="w-full flex flex-col gap-4">
          <div className="text-md bg-gray-200 text-black p-3 rounded-md border w-full">
            Concevez vos propre graphique et croisé les données officiel que
            vous souhaitez !
          </div>
        ...
        <div className="w-full flex flex-col gap-6 lg:mt-0">
          ...
          <div className="flex flex-col-reverse lg:flex-row-reverse gap-4 items-start">
            ...
            <div className="w-full lg:w-1/4 text-md text-slate-800 flex flex-col gap-4 leading-snug">
              ...
              <span>
                L'Île-de-France domine l'offre sociale pour absorber la demande,
                tandis que les zones rurales conservent des taux très inférieurs
                en raison de l'habitat individuel.
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col lg:flex-row-reverse gap-4 items-start">
            <div className="w-full lg:w-1/4 text-sm text-slate-800 flex flex-col gap-1 leading-snug">
              <span className="font-extrabold text-2xl text-black">
                Densité vs Offre Sociale
              </span>
              <span>
                Les métropoles denses concentrent l'essentiel du parc social.
                Les territoires ruraux, peu denses, restent largement en dessous
                du seuil SRU de 25%
              </span>
            </div>
